// ============================================================
// pages/CodeScan.tsx — Version corrigée (React #185 fix)
// ============================================================
import { useState, useCallback, useRef } from "react";
import { useNavigate } from "react-router-dom";
import {
  FolderOpen, GitBranch, Shield, Zap, ArrowLeft,
  Loader2, AlertTriangle, Code2, Play, Upload,
  Globe, CheckCircle2, X, FileArchive,
} from "lucide-react";
...
const SERVICES = [
  {
    ...
    icon: Shield, iconClass: "text-blue-400",
  ...
  },
  {
    ...
    icon: Code2, iconClass: "text-teal-400",
  ...
  },
  {
    ...
    icon: Zap, iconClass: "text-red-400",
...
export function CodeScan() {
  ...
  const canScan = () => {
    ...
    if (service === "sast") {
      ...
      if (sastMode === "upload") return !!sastFile && sastFile.name.toLowerCase().endsWith(".zip");
    }
    if (service === "dast") {
      if (dastMode === "preset") return true;
      if (dastMode === "custom") return customTargetUrl.startsWith("http://") || customTargetUrl.startsWith("https://");
      if (dastMode === "upload") return !!uploadFile && uploadFile.name.toLowerCase().endsWith(".zip");
    }
    if (service === "cicd") {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      if (service === "dast") {
        ...
        if (dastMode === "preset") {
          ...
          res = await dastAPI.startSync({ target_url: customTargetUrl.trim(), deploy_target: false });
        } else if (dastMode === "upload" && uploadFile) {
          res = await uploadAndScanDast(uploadFile);
          // Marquer le type pour l'affichage résultat inline
          if (res && typeof res === "object") res._type = "dast_upload";
        }

        if (dastMode === "upload") {
          // Affichage inline pour le mode upload
          setResult(res);
          setStatus("done");
        } else {
    ...
    service, sastMode, sastFile, localPath, projectName,
    dastMode, dastTarget, customTargetUrl, uploadFile,
    cicdMode, gitUrl, branch, pipelinePayload, navigate,
  ]);
  ...
  if (status === "done" && result?._type === "dast_upload") {
    ...
    return (
      <div className="p-6 max-w-2xl space-y-4">
        <button
          onClick={() => { setStatus("idle"); resetResult(); }}
        ...
        <div className={cn(
          ...
          <div>
            <p className="text-sm font-semibold">
              {isErr
                ? `Erreur : ${errMsg}`
                : `Scan terminé — ${totalVulns} vulnérabilité${totalVulns !== 1 ? "s" : ""} détectée${totalVulns !== 1 ? "s" : ""}`}
            </p>
            {up.target_url && (
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-mono text-xs text-blue-400">{safeStr(up.container_name)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Image Docker</p>
                  <p className="font-mono text-xs text-violet-400">{safeStr(up.image_name)}</p>
                </div>
                <div>
                ...
                <div className="space-y-1 border-t border-cyber-border/30 pt-3">
                  ...
                  {Object.entries(phases).map(([key, phase]: [string, any]) => {
                    ...
                    const phaseErr = phase?.error
                      ? safeStr(phase.error)
                      : null;
                    const urlCount = typeof phase?.urls_found === "number"
                      ? phase.urls_found
                      : null;
                    const vulnCount = typeof phase?.vuln_count === "number"
                      ? phase.vuln_count
                      : null;
                    ...
                    return (
                      <div
                        ...
                        className="flex items-center gap-2 text-xs py-1.5
                          border-b border-cyber-border/20 last:border-0"
                      >
                        <span className={cn(
                        ...
                        <span className="text-muted-foreground capitalize flex-1">{label}</span>
                        {urlCount  !== null && (
                          <span className="text-[10px] text-muted-foreground/60">{urlCount} URLs</span>
                        )}
                        {vulnCount !== null && (
                          <span className="text-[10px] text-amber-400">{vulnCount} vulns</span>
                        )}
                        {phaseErr && (
    ...
    <div className="max-w-5xl space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {SERVICES.map(svc => {
          ...
          return (
            <button
              key={svc.id}
              onClick={() => { setService(svc.id); setStatus("idle"); resetResult(); }}
              className={cn(
        ...
        <CardContent className="space-y-5">
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">{m.icon}
              ...
              <div>
                <label className="mb-1.5 block text-xs text-muted-foreground">Nom du projet</label>
                <input
                  value={projectName}
                  onChange={e => setProjectName(e.target.value)}
                  placeholder="mon-projet"
                  className="w-full rounded-lg border border-cyber-border bg-card/30
                    px-3 py-2 text-sm focus:border-blue-500/50 focus:outline-none"
                />
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-cyber-border bg-card/30
                        py-2 pl-8 pr-3 text-sm font-mono focus:border-blue-500/50 focus:outline-none"
                    />
                  </div>
              ...
              {sastMode === "upload" && (
                <div className="space-y-3">
                  <input
                    ...
                    onChange={e => { const f = e.target.files?.[0]; if (f) setSastFile(f); }}
                  />
                  <div
                    onClick={() => sastFileInputRef.current?.click()}
                    onDragOver={e => { e.preventDefault(); setIsSastDragging(true); }}
                    onDragLeave={() => setIsSastDragging(false)}
                    onDrop={e => {
                      e.preventDefault(); setIsSastDragging(false);
                      const f = e.dataTransfer.files?.[0]; if (f) setSastFile(f);
                    }}
                    className={cn(
                      "cursor-pointer rounded-xl border-2 border-dashed p-8 text-center transition-all",
                      isSastDragging ? "border-blue-500/70 bg-blue-500/8" :
                      sastFile       ? "border-green-500/50 bg-green-500/5" :
                    ...
                    {sastFile ? (
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="text-sm font-medium text-green-300">{sastFile.name}</p>
                          <p className="text-xs text-muted-foreground">
                            {(sastFile.size / 1024).toFixed(0)} KB
                          </p>
                        </div>
                        <button
                          onClick={e => { e.stopPropagation(); setSastFile(null);
                            if (sastFileInputRef.current) sastFileInputRef.current.value = ""; }}
                      ...
                      <>
                        ...
                        <p className="text-sm font-medium text-blue-300/80">Glissez votre projet ZIP ici</p>
                        <p className="mt-1 text-xs text-muted-foreground">ZIP uniquement · cliquez pour parcourir</p>
                      </>
                    )}
                  </div>
                </div>
          ...
          {service === "cicd" && (
            <>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { id: "repo" as CicdMode, label: "Repo GitHub",
                    icon: <GitBranch size={16} className="text-teal-400" />,
                    desc: "Cloner et scanner un dépôt" },
                  { id: "results" as CicdMode, label: "Résultats pipeline",
              ...
              {cicdMode === "repo" && (
                <>
                  <div>
                    <label className="mb-1.5 block text-xs text-muted-foreground">URL GitHub</label>
                    <div className="relative">
                      <GitBranch size={13}
                        className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                      <input
                        ...
                        className="w-full rounded-lg border border-cyber-border bg-card/30
                          py-2 pl-8 pr-3 text-sm font-mono focus:border-teal-500/50 focus:outline-none"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="mb-1.5 block text-xs text-muted-foreground">Branche</label>
                    <input
                      value={branch}
                      onChange={e => setBranch(e.target.value)}
                      placeholder="main"
                      className="w-full rounded-lg border border-cyber-border bg-card/30
                        px-3 py-2 text-sm font-mono focus:border-teal-500/50 focus:outline-none"
                    />
                  </div>
                </>
              )}
              ...
              {cicdMode === "results" && (
                <div>
                  ...
                  <textarea
                    ...
                    rows={12}
                    className="w-full rounded-lg border border-cyber-border bg-card/30
                      px-3 py-3 text-sm font-mono focus:border-teal-500/50 focus:outline-none"
                  />
                </div>
              )}
            </>
          ...
          {service === "dast" && (
            <>
              ...
              {dastMode === "custom" && (
                <div>
                  <label className="mb-1.5 block text-xs text-muted-foreground">URL cible</label>
                  <input
                    value={customTargetUrl}
                    onChange={e => setCustomTargetUrl(e.target.value)}
                    placeholder="http://cybersentinel_monapp:8080"
                    className="w-full rounded-lg border border-cyber-border bg-card/30
                      px-3 py-2 text-sm font-mono focus:border-red-500/50 focus:outline-none"
                  />
                  <p className="mt-1.5 text-xs text-muted-foreground">
              ...
              {dastMode === "upload" && (
                <div className="space-y-3">
                  ...
                  <div
                    ...
                    className={cn(
                      "cursor-pointer rounded-xl border-2 border-dashed p-8 text-center transition-all",
                      isDragging   ? "border-red-500/70 bg-red-500/8" :
                      uploadFile   ? "border-green-500/50 bg-green-500/5" :
                  ...
                  >
                    {uploadFile ? (
                      <div className="flex items-center justify-center gap-3">
                        <CheckCircle2 size={18} className="text-green-400" />
                        <div className="text-left">
                          <p className="text-sm font-medium text-green-300">{uploadFile.name}</p>
                          <p className="text-xs text-muted-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {[
                        ...
                        <div key={s.label}
                          ...
                          <span>{s.label}</span>
                          <span className="font-mono text-[10px] opacity-60">:{s.port}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2 p-3 rounded-lg
                    bg-amber-500/8 border border-amber-500/20 text-xs text-amber-400">
      ...
      {status === "error" && error && (
        <div className="flex items-center gap-3 rounded-xl border
          border-red-500/30 bg-red-500/8 p-4 text-red-300">
      ...
      <Button
        ...
        className={cn("h-11 w-full gap-2 text-sm font-semibold text-white",
      ...
      {status === "idle" && (
        <p className="text-center text-xs text-muted-foreground">
          ...
          {service === "dast" && dastMode === "preset" && "OWASP ZAP · 6 phases · 10-20 min"}
          {service === "dast" && dastMode === "custom" && "OWASP ZAP sur votre app · 10-20 min"}
          {service === "dast" && dastMode === "upload" && "Upload → build Docker → ZAP → teardown · 5-15 min"}
        </p>
      )}
    </div>
  );
